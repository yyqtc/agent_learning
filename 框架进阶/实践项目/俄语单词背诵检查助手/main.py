...
def _wait_for_new_email(mail_server):
    print("开始监听邮箱...")

    try:
        mail_server.idle()

        responses = mail_server.idle_check(timeout=1)

        if responses:
            print("服务器通知", responses)
            if any(b'EXISTS' in response for response in responses):
                print("检测到新邮件")
                return True
...
def main():
    ...
    while True:
        if _wait_for_new_email(mail_server):
